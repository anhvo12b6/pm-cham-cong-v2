...
const checkRole = (allowedRoles) => {
    return (req, res, next) => {
        ...
        if (!hasRole) {
            return res.status(403).json({ message: "Tài khoản không có quyền truy cập chức năng này" });
        }
        next();
    };
};
...
const handleSSOLogin = async (req, res) => {
    ...
    try {
        ...
        const decodedSSO = jwt.verify(token, process.env.JWT_KEY || 'CHẤM CÔNG APP V2');
        const username = decodedSSO.username;

        if (!username) {
            return res.status(400).send("SSO Token invalid: username field is missing");
        }
        ...
        if (!user) {
            ...
            return res.redirect(`${clientUrl}?sso_error=${encodeURIComponent(`Không tìm thấy tài khoản '${username}' trong hệ thống!`)}`);
        }
...
app.get('/api/bao-cao/phong-ban', authenticate, async (req, res) => {
    ...
    try {
        ...
        if (isManager && !isAdmin) {
            ...
            if (allowedKhuVuc.length === 0 && allowedPhongBan.length === 0) {
                return res.status(403).json({ message: "Tài khoản của bạn chưa được phân quyền xem bất kỳ bộ phận nào!" });
            }
            ...
            if (maPhongBan === 'ALL') {
                ...
                const kvResult = await pool.request()
        ...
        for (const emp of employees) {
            for (const dateObj of datesInRange) {
                ...
                if (punches.length === 1) {
                    ...
                    TrangThai = lateInfo.isLate ? `Trễ ${lateInfo.minutes} phút (Thiếu ra)` : "Đúng giờ (Thiếu ra)";
                    KyHieu = "O";
                    KhuVucVao = punches[0].TenMay ? punches[0].TenMay.trim() : `Máy ${punches[0].MaSoMay}`;
                    ...
                    const lateInfo = checkIsLate(GioVao);
                    TrangThai = lateInfo.isLate ? `Trễ ${lateInfo.minutes} phút` : "Đúng giờ";
                    KhuVucVao = punches[0].TenMay ? punches[0].TenMay.trim() : `Máy ${punches[0].MaSoMay}`;
                    KhuVucRa = punches[punches.length - 1].TenMay ? punches[punches.length - 1].TenMay.trim() : `Máy ${punches[punches.length - 1].MaSoMay}`;
                }
...
app.get('/api/bao-cao/export-excel', authenticate, async (req, res) => {
    ...
    try {
        ...
        if (maPhongBan === 'ALL') {
            ...
            maPhongBanList = [maPhongBan.trim()];
            const pbResult = await pool.request()
                .input('MaPhongBan', sql.VarChar(50), maPhongBan)
                .query(`
        ...
        worksheet.getCell('A3').font = { name: 'Times New Roman', bold: true, size: 11 };

        // Dòng 4: Metadata Phòng ban
        worksheet.getCell('A4').value = `BỘ PHẬN/XƯỞNG: ${tenPhongBan}`;
        worksheet.getCell('A4').font = { name: 'Times New Roman', bold: true, size: 11 };
        worksheet.getRow(3).height = 20;
        worksheet.getRow(4).height = 20;

        // Dòng 5, 6, 7: Các tiêu đề bảng công (Headers)
        worksheet.mergeCells(5, 1, 7, 1); worksheet.getCell(5, 1).value = 'STT';
        worksheet.mergeCells(5, 2, 7, 2); worksheet.getCell(5, 2).value = 'MÃ NHÂN VIÊN';
        worksheet.mergeCells(5, 3, 7, 3); worksheet.getCell(5, 3).value = 'HỌ VÀ TÊN';
        worksheet.mergeCells(5, 4, 7, 4); worksheet.getCell(5, 4).value = 'Ngày Vào Làm';
        ...
        worksheet.mergeCells(5, endDayColIdx + 1, 7, endDayColIdx + 1); worksheet.getCell(5, endDayColIdx + 1).value = 'Công Chính';
        worksheet.mergeCells(5, endDayColIdx + 2, 7, endDayColIdx + 2); worksheet.getCell(5, endDayColIdx + 2).value = 'Tăng Ca';
        worksheet.mergeCells(5, endDayColIdx + 3, 7, endDayColIdx + 3); worksheet.getCell(5, endDayColIdx + 3).value = 'Chủ Nhật';
        worksheet.mergeCells(5, endDayColIdx + 4, 7, endDayColIdx + 4); worksheet.getCell(5, endDayColIdx + 4).value = 'Tăng Ca Chủ Nhật';
        worksheet.mergeCells(5, endDayColIdx + 5, 7, endDayColIdx + 5); worksheet.getCell(5, endDayColIdx + 5).value = 'Ghi Chú';
        ...
        employees.forEach((emp, empIdx) => {
            ...
            datesInRange.forEach((d, idx) => {
                ...
                if (punches.length === 1) {
                    ...
                    const diffHours = (GioRa.getTime() - GioVao.getTime()) / (1000 * 60 * 60);
            ...
            worksheet.mergeCells(startRow, endDayColIdx + 1, endRow, endDayColIdx + 1);
            worksheet.getCell(startRow, endDayColIdx + 1).value = totalRegularWork;

            worksheet.mergeCells(startRow, endDayColIdx + 2, endRow, endDayColIdx + 2);
            worksheet.getCell(startRow, endDayColIdx + 2).value = totalRegularOvertime;

            worksheet.mergeCells(startRow, endDayColIdx + 3, endRow, endDayColIdx + 3);
            worksheet.getCell(startRow, endDayColIdx + 3).value = totalSundayWork;

            worksheet.mergeCells(startRow, endDayColIdx + 4, endRow, endDayColIdx + 4);
            worksheet.getCell(startRow, endDayColIdx + 4).value = totalSundayOvertime;

            worksheet.mergeCells(startRow, endDayColIdx + 5, endRow, endDayColIdx + 5);
            ...
            worksheet.getCell(startRow, 3).alignment = { horizontal: 'left', vertical: 'middle' };
        ...
        worksheet.getCell(totalRowIdx, 1).font = { name: 'Times New Roman', bold: true, size: 9 };
        worksheet.getCell(totalRowIdx, 1).alignment = { horizontal: 'center', vertical: 'middle' };
        ...
        worksheet.getCell(sigRowIdx, 3).font = { name: 'Times New Roman', bold: true, size: 10 };
        worksheet.getCell(sigRowIdx, 3).alignment = { horizontal: 'center', vertical: 'middle' };

        // Khối Giám đốc sản xuất
        const midCol = Math.floor(totalCols / 2);
        worksheet.getCell(sigRowIdx, midCol).value = 'GIÁM ĐỐC SẢN XUẤT';
        worksheet.getCell(sigRowIdx, midCol).font = { name: 'Times New Roman', bold: true, size: 10 };
        worksheet.getCell(sigRowIdx, midCol).alignment = { horizontal: 'center', vertical: 'middle' };

        // Khối Người chấm công
        worksheet.getCell(sigRowIdx, totalCols - 2).value = 'NGƯỜI CHẤM CÔNG';
        worksheet.getCell(sigRowIdx, totalCols - 2).font = { name: 'Times New Roman', bold: true, size: 10 };
        worksheet.getCell(sigRowIdx, totalCols - 2).alignment = { horizontal: 'center', vertical: 'middle' };
...
app.get('/api/bao-cao/export-excel-detail', authenticate, async (req, res) => {
    ...
    try {
        ...
        const headers = [
            'STT', 'Mã nhân viên', 'Tên nhân viên', 'Phòng ban', 'Khu vực', 
            'Ngày', 'Thứ', 'Giờ vào', 'Giờ ra', 'Trễ', 
            'Sớm', 'Công', 'Tổng giờ', 'Tăng ca', 'Tổng toàn bộ', 'Ca'
        ];
        ...
        employees.forEach((emp) => {
            datesInRange.forEach((d) => {
                ...
                newRow.getCell(3).alignment = { horizontal: 'left', vertical: 'middle' }; // Tên nv
                newRow.getCell(4).alignment = { horizontal: 'left', vertical: 'middle' }; // Phòng ban
            });
        });
...
app.get('/api/admin/roles', authenticate, checkRole(['Admin']), async (req, res) => {
    try {
        const pool = await authPool;
        const result = await pool.request().query(`
...
app.get('/api/admin/employees', authenticate, checkRole(['Admin']), async (req, res) => {
    try {
        const pool = await mitacoPool;
        const result = await pool.request().query(`
...
app.post('/api/admin/update-user-auth', authenticate, checkRole(['Admin']), async (req, res) => {
    const { userId, maChamCong, roleIds, allowedKhuVuc, allowedPhongBan } = req.body;

    if (!userId || !Array.isArray(roleIds)) {
        return res.status(400).json({ message: "Thiếu thông tin người dùng hoặc vai trò!" });
    }
    ...
    try {
        ...
        await requestUsers
            ...
            .input('AllowedKhuVuc', sql.NVarChar(sql.MAX), allowedKhuVuc ? JSON.stringify(allowedKhuVuc) : null)
            .input('AllowedPhongBan', sql.NVarChar(sql.MAX), allowedPhongBan ? JSON.stringify(allowedPhongBan) : null)
            .query(`
                UPDATE Web_Users 
...
app.post('/api/admin/create-user', authenticate, checkRole(['Admin']), async (req, res) => {
    const { username, fullName, password, maChamCong, roleIds, allowedKhuVuc, allowedPhongBan } = req.body;
